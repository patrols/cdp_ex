defmodule CDPEx.ProcessLabelTest do
  ...
  # ProcessLabel.set/1 is a no-op and there is nothing to assert.
  #
  # Skipped at runtime rather than compiled away: an `if supported? do` around the
  # body puts every alias in a dead branch, which 1.15 reports as unused — and
  # dropping the aliases to silence that trades it for 15 Credo AliasUsage
  # findings. Compiling unconditionally is the only shape both agree on.
  supported? =
  ...
  # `apply/3`, not a direct call: `:proc_lib.get_label/1` does not exist on OTP 26,
  # and naming it directly warns at compile time even though these tests never run
  # there.
  defp label_of(pid), do: apply(:proc_lib, :get_label, [pid])
  ...
  describe "CDPEx.Pool" do
    ...
    test "labels each browser-launch task with the pool it serves" do
      ...
      start_fun = fn opts ->
        send(test_pid, {:launch_label, apply(:proc_lib, :get_label, [self()])})
