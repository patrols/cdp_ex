...
### Added
- Process labels on every GenServer and task CDPEx starts, so a pid in a crash report, `:observer`, or Phoenix LiveDashboard identifies itself: `{:cdp_browser, os_pid}` (the Chrome OS pid; `:starting` before it is known, `:connected` in connect-mode, `:launched` when the OS pid is unavailable), `{:cdp_connection, target}` (the DevTools path, which distinguishes the browser socket from a dedicated page socket; the query string is stripped since it can carry an auth token), `{:cdp_page_helper, :idle_wait | :capture, target_id}` for the helpers behind `wait_for_network_idle/2` and `navigate/3` with `response: true`, `{:cdp_fetch_auth, source}`, `{:cdp_fetch_disable, conn}`, and `{:cdp_pool, size}` / `{:cdp_pool_launch, pool}`. The `Task.Supervisor` inside `CDPEx.Pool` stays unlabelled — OTP offers no way to label a supervisor from outside it. Labels are set on Elixir 1.17+ and compile to a no-op on 1.15/1.16; see the README's "Process labels" section (#103).
...
[Unreleased]: https://github.com/patrols/cdp_ex/compare/v0.10.0...HEAD
[0.10.0]: https://github.com/patrols/cdp_ex/compare/v0.9.0...v0.10.0
