...
### Added
- Process labels on every process CDPEx starts, so a pid in a crash report, `:observer`, or Phoenix LiveDashboard identifies itself: `{:cdp_browser, os_pid}` (the Chrome OS pid, `:connected` in connect-mode), `{:cdp_connection, target}` (the DevTools path, which distinguishes the browser socket from a dedicated page socket), `{:cdp_page_helper, :idle_wait | :capture, target_id}` for the helpers behind `wait_for_network_idle/2` and `navigate/3` with `response: true`, `{:cdp_fetch_auth, source}`, and `{:cdp_pool, size}` / `{:cdp_pool_launch, pool}`. Labels are set on Elixir 1.17+ and compile to a no-op on 1.15/1.16; see the README's "Process labels" section (#103).
