...
Every process CDPEx starts sets a [process
...
Nothing to configure; labels are set on Elixir 1.17+ and read by OTP 27+.

| Label | Process |
|-------|---------|
| `{:cdp_browser, os_pid}` | `CDPEx.Browser` — `os_pid` is the Chrome process it launched (`:connected` in connect-mode, `:starting` until Chrome is up) |
